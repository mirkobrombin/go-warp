...
func TestWarpSetSkipsStoreReadWithoutCustomMerge(t *testing.T) {
	w := New[string](cache.NewInMemory[merge.Value[string]](), errStore[string]{err: errors.New("store read")}, nil, merge.NewEngine[string]())
	w.Register("foo", ModeStrongLocal, time.Minute)
	if err := w.Set(context.Background(), "foo", "bar"); err != nil {
		t.Fatalf("set read store without custom merge: %v", err)
	}
}

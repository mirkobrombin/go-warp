...
var tracer = otel.Tracer("github.com/mirkobrombin/go-warp/v2/core")
...
func (w *Warp[T]) GetOrSet(ctx context.Context, key string, loader func(context.Context) (T, error)) (T, error) {
	...
	vInterface, errS, _ := w.group.Do(key, func() (any, error) {
		...
		mv := merge.Value[T]{Data: val, Timestamp: time.Now()}
		ttl := reg.ttl
		if reg.ttlStrategy != nil {
			ttl = reg.ttlStrategy.TTL(key)
		}
...
func (w *Warp[T]) Set(ctx context.Context, key string, value T) error {
	...
	old, found, err := w.cache.Get(ctx, key)
	if err != nil {
		return err
	}
	if !found && w.store != nil && w.merges.HasCustom(key) {
		stored, ok, err := w.store.Get(ctx, key)
		if err != nil {
			return err
		}
		if ok {
			old = merge.Value[T]{Data: stored}
			found = true
		}
	}
	if found {

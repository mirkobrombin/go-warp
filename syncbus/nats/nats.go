...
func (b *NATSBus) Publish(ctx context.Context, key string, opts ...syncbus.PublishOption) error {
	...
	for {
		...
		if next, reconnectErr := b.reconnect(); reconnectErr == nil {
			conn = next
		} else {
			err = reconnectErr
		}
...
func (b *NATSBus) reconnect() (*nats.Conn, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	resubscribed := make(map[string]*nats.Subscription, len(b.subs))
	for key := range b.subs {
		ns, err := newConn.Subscribe(key, b.natsHandler(key))
		if err != nil {
			newConn.Close()
			return b.conn, err
		}
	...
	if err := flush(context.Background(), newConn); err != nil {
		newConn.Close()
		return b.conn, err
	}
	b.conn = newConn
	for key, ns := range resubscribed {
		b.subs[key].sub = ns
	}
	return b.conn, nil
}

func flush(ctx context.Context, conn *nats.Conn) error {
